...
Within NONMEM it is also possible to write abbreviated FORTRAN code (e.g. including things like `COM(n)` and/or `$ABBREVIATED`). Additionally, the usage of protected functions in R could lead to possible issues in the translated model (e.g. `PROTOECT` option in `$ABBREVIATED`). 
This part is currently not supported by the package and should be deleted/rewritten before translating the model.
